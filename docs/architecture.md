...
When creating a new blog post, you need to generate a unique `id` for the post's frontmatter. This ID is used for Giscus comments integration.

**ID Format:** `{slug}-{hash}`

- **slug**: Title of the post formatted as a URL-safe string (lowercase, hyphens)
- **hash**: 6-character random string generated by nanoid

**Example:** `my-new-post-a3f91c`

**How to generate the ID:**

1. Use the utility function in `src/domains/blog/post-id.ts`:

```typescript
import { generatePostId } from './domains/blog/post-id';

const id = generatePostId('My New Post');
// Returns: 'my-new-post-xxxxxx'
```

2. Or manually create using the format: `{title-slug}-{6-char-hash}`
...
- The `id` must be **immutable** after creation. Changing the ID will create a new Giscus Discussion, losing existing comments.
- The build will fail if the ID doesn't match the required pattern.
